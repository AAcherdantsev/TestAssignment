...
public class IncludedBasicUnitTests
{
    ...
    public async Task ThrottlingWorksFor_SameIpGetsBanned()
    {
        ...
        Assert.True((await apiService.GetResourceAsync(new("127.0.0.1", "example@email.com", "id1"))).Success);
        Assert.Equal(1, (await apiService.GetResourceAsync(new("127.0.0.1", "example2@email.com", "id1"))).ResourceData);
        Assert.False((await apiService.GetResourceAsync(new("127.0.0.1", "example3@email.com", "id1"))).Success);
        ;
    }
    ...
    public async Task ThrottlingDoesNotBan_AfterIntervalPasses()
    {
        ...
        Assert.True((await apiService.GetResourceAsync(new("127.0.0.1", "example@email.com", "id1"))).Success);
        Assert.Equal(1, (await apiService.GetResourceAsync(new("127.0.0.1", "example@email.com", "id2"))).ResourceData);
        Assert.False((await apiService.GetResourceAsync(new("127.0.0.1", "example2@email.com", "id1"))).Success);

        timeProvider.UtcNow = DateTime.UnixEpoch + 2 * throttleSettings.ThrottleInterval;

        Assert.True((await apiService.GetResourceAsync(new("127.0.0.1", "different@email.com", "id3"))).Success);
        ;
    }
    ...
    public async Task ThrottlingDoesNotBan_SameResourceDifferentIp()
    {
        ...
        Assert.True((await apiService.GetResourceAsync(new("127.0.0.1", "example@email.com", "id1"))).Success);
        Assert.True((await apiService.GetResourceAsync(new("127.0.0.1", "example2@email.com", "id1"))).Success);
        Assert.True((await apiService.GetResourceAsync(new("127.0.0.2", "example@email.com", "id1"))).Success);
        Assert.True((await apiService.GetResourceAsync(new("127.0.0.2", "example@email.com", "id1"))).Success);
        ;
    }
    ...
    private class InjectedResourceProvider<TResource> : IResourceProvider<TResource>
    {
        ...
        public bool TryGetResource(string id, out TResource resource)
        {
            resource =  _getResource(id);
